...
const fetchFilteredAnimes = (pageUrl?: string) => {
  ...
  inertia.get(pageUrl || route('en.movies'), {
    search: search.value,
    ...
    only: ['animes'],
    onSuccess: (res) => {
      animes.value = res.props.animes.data; // الآن فقط Movie
...
const goToPage = (pageNumber: number) => {
  fetchFilteredAnimes(`${route('en.movies')}?page=${pageNumber}&search=${search.value}&status=${selectedStatus.value}&category=${selectedCategory.value}&season=${selectedSeason.value}`);
};
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col flex-1 h-full gap-4 p-4 rounded-xl">
      <span dir="rtl">قائمة - الأفلام</span>

      <!-- البحث + الفلترة -->
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-center">
        <Input v-model="search" placeholder="Search movies..." class="w-full h-10 md:w-1/3" />
        <button @click="showFilter = !showFilter" class="flex items-center gap-2 px-4 py-2 bg-white border rounded-md hover:bg-gray-100">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-6 h-6 stroke-gray-800">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3c2.755 0 5.455.232 8.083.678.533.09.917.556.917 1.096v1.044a2.25 2.25 0 0 1-.659 1.591l-5.432 5.432a2.25 2.25 0 0 0-.659 1.591v2.927a2.25 2.25 0 0 1-1.244 2.013L9.75 21v-6.568a2.25 2.25 0 0 0-.659-1.591L3.659 7.409A2.25 2.25 0 0 1 3 5.818V4.774c0-.54.384-1.006.917-1.096A48.32 48.32 0 0 1 12 3Z"/>
          </svg>
          <span class="font-medium text-black">Filter</span>
        </button>
        <button @click="resetFilters" class="px-4 py-2 text-gray-900 bg-white border rounded-md hover:bg-gray-100">Reset All</button>
      </div>
      ...
      <transition name="fade-slide">
        <div v-if="showFilter" class="flex flex-wrap items-center justify-center gap-3 p-5 bg-white/80 dark:bg-[#292929cc] backdrop-blur-md rounded-xl shadow-lg transition-all duration-300">
          <select v-model="selectedStatus" class="px-4 py-2 border rounded-lg dark:bg-gray-900 dark:text-white">
            <option value="">All Status</option>
     <option value="Active">Currently Showing</option>
<option value="completed">Completed</option>

          </select>

          <select v-model="selectedCategory" class="px-4 py-2 border rounded-lg dark:bg-gray-900 dark:text-white">
            <option value="">All Categories</option>
            <option v-for="category in categories" :key="category.id" :value="category.name_en">{{ category.name_en }}</option>
          </select>

          <select v-model="selectedSeason" class="px-4 py-2 border rounded-lg dark:bg-gray-900 dark:text-white">
      ...
      <div class="flex items-center justify-center gap-2 mt-6">
        <button :disabled="currentPage === 1" @click="goToPage(currentPage - 1)" class="px-3 py-1 border rounded disabled:opacity-50">Prev</button>
        <span class="px-2">{{ currentPage }} / {{ lastPage }}</span>
        <button :disabled="currentPage === lastPage" @click="goToPage(currentPage + 1)" class="px-3 py-1 border rounded disabled:opacity-50">Next</button>
      </div>
    </div>
  </AppLayout>
</template>
