...
import { 
    ...
    History,     // أيقونة السجل
    Clock,       // أيقونة المشاهدة لاحقا
    Moon,        // أيقونة الوضع المظلم
    Sun,         // أيقونة الوضع النهاري
    Loader2,     // أيقونة التحميل Spinner
    X,           // زر إغلاق او حذف
    MoreVertical // خيارات اضافية
...
const handleOpenNotifications = async (isOpen: boolean) => {
    if (isOpen) {
        ...
        // محاكاة طلب للسيرفر (استبدلها بـ axios.get('/api/notifications'))
        setTimeout(() => {
            notifications.value = [
                ...
                { id: 4, title: 'تحديث أمني جديد لحسابك', time: 'أمس', image: null, icon: 'system', read: true },
            ];
            loadingNotifications.value = false;
        }, 1500); // تأخير 1.5 ثانية لظهور الـ Spinner
    }
};

// --- إدارة سجل المشاهدة (History) ---
    ...
    if (isOpen) {
        ...
        setTimeout(() => {
            watchHistory.value = [
                ...
                { id: 103, title: 'Dr. Stone: New World', episode: 'الحلقة 9', progress: 100, image: 'https://cdn.myanimelist.net/images/anime/1105/133642.jpg' },
            ];
...
// --- إدارة المشاهدة لاحقاً (Watch Later) ---
...
const mainNavItems: NavItem[] = [
  { title: 'الرئيسية', href: '/ar/home', icon: Home }, // أيقونة المنزل
  { title: 'أنمي القادم', href: '/ar/coming-soon', icon: CalendarClock }, // أيقونة القادم
...
// روابط يمين (تم تقليلها لأننا أضفنا أزرار جديدة)
const rightNavItems: NavItem[] = []; // تركتها فارغة لأننا أضفنا الأيقونات الجديدة مباشرة في الـ Template
